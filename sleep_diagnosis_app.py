...
def render_highlighted_result(text: str) -> None:
    """診断結果の重要な文言を太字と黄色い下線で強調表示する。"""
    st.markdown(
        f"""
        <div style="
            font-size: 1.4rem;
...
with st.form("sleep_check_form"):
    ...
    sleeping_time = st.number_input(
        "平均的な睡眠時間（時間）",
        min_value=0.0,
        max_value=24.0,
        value=7.0,
        step=0.5,
        help="過去2週間の平均的な睡眠時間を入力してください。",
    )
...
if submitted:
    ...
    render_highlighted_result(f"総合評価：{severity_grade}評価")

    render_highlighted_result(severity_title)
    st.write(severity_message)

    render_highlighted_result(f"主な睡眠タイプ：{sleep_type_title}")
    st.write(sleep_type_message)
    render_highlighted_result(f"最も気になっている悩み：{main_sleep_problem}")
    ...
    for number, advice in enumerate(sleep_advice, start=1):
        render_highlighted_result(f"{number}．{advice}")
